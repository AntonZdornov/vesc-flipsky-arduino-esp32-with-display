...
#define TP_REG_FINGER_NUM  0x02  // далее подряд: XH, XL, YH, YL
#define TP_REG_CHIP_ID     0xA7
#define TP_REG_DIS_AUTO_SLEEP 0xFE  // 0x01 = не уходить в сон
...
bool Touch_Init(void) {
  // INT читаем как обычный вход: чип опрашивается поллингом из read_cb LVGL,
  // прерывание не нужно (I2C-чтение само выводит CST816T из сна).
  ...
  delay(60);  // чипу нужно ~50 мс на инициализацию после reset
  ...
  // Без этого чип засыпает и первые касания теряются
...
bool Touch_Read(uint16_t *x, uint16_t *y) {
  ...
  uint8_t d[5];  // [0]=кол-во касаний, [1..2]=X, [3..4]=Y
  ...
  // Тач-панель отдаёт координаты в нативной портретной системе (240x280),
  // она же логическая при LCD_PORTRAIT=1. Для ландшафта нужен своп/зеркало.
#if LCD_PORTRAIT
  *x = rx;
  *y = ry;
#else
